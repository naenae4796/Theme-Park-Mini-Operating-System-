...
The output is paced for live demos. By default, each simulated minute waits 5000 ms, so the three scheduler demos finish in about 5-10 minutes. For quick testing, use `SIM_TICK_MS=0 ./build/theme_park_os` on Linux/WSL/macOS or `set SIM_TICK_MS=0` before running on Windows.
